...
form.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    resultText.textContent = `Welcome, ${data.name}! You chose ${prettyDate(data.date_day)}. You are currently #${position} on the scoreboard. 🏆`;
...
showPaymentBtn.addEventListener("click", () => {
  ...
  paymentMessage.textContent = "🤨 Ahhh, so we're trying to buy our way to the top now?";
...
payBtn.addEventListener("click", () => {
  const jokes = [
    ...
    "🚨 BANK ERROR: Your girlfriend application has detected suspicious levels of desperation.",
    "😂 TRANSACTION FAILED: Nice try. The scoreboard remains undefeated.",
    "💔 PAYMENT REJECTED: Money can't buy love... but points would have been funny.",
    "🤨 PAYMENT FAILED: The CEO of Girlfriend Applications has personally rejected this transaction."
  ];

  paymentMessage.textContent = jokes[Math.floor(Math.random() * jokes.length)];
  payBtn.textContent = "😭 Payment Failed";

  setTimeout(() => {
    payBtn.textContent = "💳 Pay & Buy My Position";
  }, 2200);
});
